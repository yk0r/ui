...
---------------------------------------------------------------------
-- Component Beautification Patch (non-invasive)
-- Imports Notify animations, Search open/close effects, Hide/Unhide/Maximise
-- from skgmain style without changing themes or window logic.
---------------------------------------------------------------------

task.spawn(function()
    -- graceful wait for game & UI
    pcall(function() repeat task.wait() until game:IsLoaded() end)
    local CoreGui = game:GetService("CoreGui")
    local TweenService = game:GetService("TweenService")

    -- Try resolve core references from original Rayfield
    local Rayfield = (gethui and gethui():FindFirstChild("Rayfield")) or CoreGui:FindFirstChild("Rayfield")
    ...
    -- Try get RayfieldLibrary table from global/upvalue
    local RayfieldLibrary = rawget(getfenv(0), "RayfieldLibrary")
    if not RayfieldLibrary then
        -- attempt from shared
        RayfieldLibrary = shared and shared.RayfieldLibrary or nil
    end
    if not RayfieldLibrary then return end

    -----------------------------------------------------------------
    -- Messaging System for Notifications (cloned container)
    -----------------------------------------------------------------
    local MessagingSystem = Instance.new("ScreenGui")
    MessagingSystem.Name = "MESSAGINGSYSTEM"
    MessagingSystem.ResetOnSpawn = false
    MessagingSystem.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    MessagingSystem.Parent = CoreGui

    local OriginalNotifications = Rayfield:FindFirstChild("Notifications")
    local VisibleNotifications
    if OriginalNotifications then
        VisibleNotifications = OriginalNotifications:Clone()
        VisibleNotifications.Parent = MessagingSystem
        VisibleNotifications.Enabled = true
        VisibleNotifications.Name = "Notifications"
    end

    -----------------------------------------------------------------
    -- Enhanced Notify (non-theme, animation only)
    -----------------------------------------------------------------
    function RayfieldLibrary:Notify(data)
        task.spawn(function()
            if not VisibleNotifications then return end
            local template = VisibleNotifications:FindFirstChild("Template")
            if not template then return end
            local n = template:Clone()
            n.Name = data.Title or "Notification"
            n.Parent = VisibleNotifications
            n.Visible = false

            -- set texts/icons (keep existing colors from clone)
            n.Title.Text = data.Title or "Notification"
            n.Description.Text = data.Content or ""
            if data.Image then
                n.Icon.Image = "rbxassetid://"..tostring(data.Image)
                n.Icon.Visible = true
            end
            ...
            if n:FindFirstChild("Shadow") and n.Shadow:IsA("ImageLabel") then
                n.Shadow.ImageTransparency = 1
            end

            -- autosize to text
            task.wait()
            local boundsY = (n.Title.TextBounds.Y + n.Description.TextBounds.Y)
            n.Size = UDim2.new(1, -60, 0, math.max(boundsY + 31, 60))

            n.Visible = true
            TweenService:Create(n, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.45}):Play()
            TweenService:Create(n.UIStroke, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {Transparency = 0.1}):Play()
            TweenService:Create(n.Title, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()
            TweenService:Create(n.Icon, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = 0}):Play()
            TweenService:Create(n.Description, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {TextTransparency = 0.2}):Play()
            if n:FindFirstChild("Shadow") and n.Shadow:IsA("ImageLabel") then
                TweenService:Create(n.Shadow, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {ImageTransparency = 0.82}):Play()
            end

            local waitDuration = math.min(math.max((#n.Description.Text * 0.06) + 2.5, 3), 8)
            task.wait(data.Duration or waitDuration)

            -- fade out
            TweenService:Create(n, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {BackgroundTransparency = 1}):Play()
            TweenService:Create(n.UIStroke, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
            TweenService:Create(n.Title, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {TextTransparency = 1}):Play()
            TweenService:Create(n.Description, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {TextTransparency = 1}):Play()
            TweenService:Create(n.Icon, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = 1}):Play()
            if n:FindFirstChild("Shadow") and n.Shadow:IsA("ImageLabel") then
                TweenService:Create(n.Shadow, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {ImageTransparency = 1}):Play()
            end
            task.wait(0.3)
            n:Destroy()
        end)
    ...
    -----------------------------------------------------------------
    -- Search open/close animation hooks
    -----------------------------------------------------------------
    local searchOpen = false
    local function openSearch()
        if not Main:FindFirstChild("Search") then return end
        ...
        if Search:FindFirstChild("Input") then
            ...
            pcall(function() Search.Input.Interactable = true end)
        end
        if Search:FindFirstChild("Search") then Search.Search.ImageTransparency = 1 end

        TweenService:Create(Search, TweenInfo.new(0.28, Enum.EasingStyle.Exponential), {
            BackgroundTransparency = 0.9, Size = UDim2.new(1, -35, 0, 35)
        }):Play()
        if Search:FindFirstChild("Shadow") then
            TweenService:Create(Search.Shadow, TweenInfo.new(0.05, Enum.EasingStyle.Quint), {ImageTransparency = 0.95}):Play()
        end
        TweenService:Create(Search.UIStroke, TweenInfo.new(0.28, Enum.EasingStyle.Exponential), {Transparency = 0.2}):Play()
        if Search:FindFirstChild("Input") then
            TweenService:Create(Search.Input, TweenInfo.new(0.28, Enum.EasingStyle.Exponential), {TextTransparency = 0.1}):Play()
            pcall(function() Search.Input:CaptureFocus() end)
        end
        if Search:FindFirstChild("Search") then
            TweenService:Create(Search.Search, TweenInfo.new(0.28, Enum.EasingStyle.Exponential), {ImageTransparency = 0.2}):Play()
        end

        -- fade tab buttons while searching
        if TabList then
            for _, tabbtn in ipairs(TabList:GetChildren()) do
                if tabbtn:IsA("Frame") and tabbtn.Name ~= "Placeholder" then
                    ...
                    TweenService:Create(tabbtn, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {BackgroundTransparency = 1}):Play()
                    if tabbtn:FindFirstChild("Title") then
                        TweenService:Create(tabbtn.Title, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {TextTransparency = 1}):Play()
                    end
                    if tabbtn:FindFirstChild("Image") then
                        TweenService:Create(tabbtn.Image, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = 1}):Play()
                    end
                    if tabbtn:FindFirstChild("UIStroke") then
                        TweenService:Create(tabbtn.UIStroke, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
                    end
                end
            end
        end
    ...
    local function closeSearch()
        ...
        TweenService:Create(Search, TweenInfo.new(0.25, Enum.EasingStyle.Quint), {
            BackgroundTransparency = 1, Size = UDim2.new(1, -55, 0, 30)
        }):Play()
        if Search:FindFirstChild("Search") then
        ...
        if Search:FindFirstChild("UIStroke") then
        ...
        if Search:FindFirstChild("Input") then
            ...
            Search.Input.Text = ""
            pcall(function() Search.Input.Interactable = false end)
            pcall(function() Search.Input:ReleaseFocus() end)
        end

        -- restore tabs appearance
        if TabList and Elements and Elements:FindFirstChild("UIPageLayout") then
            local current = Elements.UIPageLayout.CurrentPage
            for _, tabbtn in ipairs(TabList:GetChildren()) do
                if tabbtn:IsA("Frame") and tabbtn.Name ~= "Placeholder" then
                    if tabbtn:FindFirstChild("PageHolder") and tabbtn.PageHolder.Value == current then
                        TweenService:Create(tabbtn, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0}):Play()
                        if tabbtn:FindFirstChild("Image") then
                            TweenService:Create(tabbtn.Image, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = 0}):Play()
                        end
                        if tabbtn:FindFirstChild("Title") then
                            TweenService:Create(tabbtn.Title, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()
                        end
                        if tabbtn:FindFirstChild("UIStroke") then
                            TweenService:Create(tabbtn.UIStroke, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
                        end
                    else
                        TweenService:Create(tabbtn, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.7}):Play()
                        if tabbtn:FindFirstChild("Image") then
                            TweenService:Create(tabbtn.Image, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = 0.2}):Play()
                        end
                        if tabbtn:FindFirstChild("Title") then
                            TweenService:Create(tabbtn.Title, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {TextTransparency = 0.2}):Play()
                        end
                        if tabbtn:FindFirstChild("UIStroke") then
                            TweenService:Create(tabbtn.UIStroke, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {Transparency = 0.5}):Play()
                        end
                    end
    ...
    -- Hook topbar search button if present
    pcall(function()
        if Topbar and Topbar:FindFirstChild("Search") and Topbar.Search:IsA("ImageButton") then
            Topbar.Search.MouseButton1Click:Connect(function()
                if not searchOpen then openSearch() else closeSearch() end
            end)
        end
    end)

    -----------------------------------------------------------------
    -- Hide/Unhide/Maximise animations (non-invasive)
    -----------------------------------------------------------------
    local Debounce, Minimised = false, false

    function RayfieldLibrary:Hide(notify)
        if Debounce then return end
        Debounce = true
        pcall(closeSearch)

        local function tween(o, ti, props) if o then TweenService:Create(o, ti, props):Play() end end
        tween(Main, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 470, 0, 0), BackgroundTransparency = 1})
        if Topbar then
            tween(Topbar, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 470, 0, 45), BackgroundTransparency = 1})
            if Topbar:FindFirstChild("Divider") then tween(Topbar.Divider, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 1}) end
            if Topbar:FindFirstChild("Title") then tween(Topbar.Title, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {TextTransparency = 1}) end
            if Topbar:FindFirstChild("ImageLabel") then tween(Topbar.ImageLabel, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {ImageTransparency = 1}) end
            if Topbar:FindFirstChild("UIStroke") then tween(Topbar.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Transparency = 1}) end
            for _, b in ipairs(Topbar:GetChildren()) do
                if b:IsA("ImageButton") then tween(b, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {ImageTransparency = 1}) end
            end
        end
        if notify then
            self:Notify({
                Title = "Interface Hidden",
                Content = "Use your UI toggle to show it again.",
                Image = "6023426928",
                Duration = 4
            })
        end
        task.delay(0.45, function() Main.Visible = false Debounce = false end)
    end
    ...
    function RayfieldLibrary:Maximise()
        ...
        Main.Visible = true
        local function tween(o, ti, props) if o then TweenService:Create(o, ti, props):Play() end end
        tween(Main, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 500, 0, 475), BackgroundTransparency = 0})
        if Topbar then
            tween(Topbar, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 500, 0, 45), BackgroundTransparency = 0})
            if Topbar:FindFirstChild("Divider") then tween(Topbar.Divider, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0}) end
            if Topbar:FindFirstChild("Title") then tween(Topbar.Title, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {TextTransparency = 0}) end
            if Topbar:FindFirstChild("ImageLabel") then tween(Topbar.ImageLabel, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {ImageTransparency = 0}) end
            if Topbar:FindFirstChild("UIStroke") then tween(Topbar.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Transparency = 0}) end
            for _, b in ipairs(Topbar:GetChildren()) do
                if b:IsA("ImageButton") then tween(b, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {ImageTransparency = 0.8}) end
            end
        end

        -- restore elements (only visuals)
        if Elements then
            Elements.Visible = true
            for _, tab in ipairs(Elements:GetChildren()) do
                    ...
                    for _, element in ipairs(tab:GetChildren()) do
                        if element:IsA("Frame") and element.Name ~= "SectionSpacing" and element.Name ~= "Placeholder" then
                            if element:FindFirstChild("UIStroke") then
                                TweenService:Create(element.UIStroke, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {Transparency = 0}):Play()
                            end
                            TweenService:Create(element, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0}):Play()
                            if element:FindFirstChild("Title") then
                                TweenService:Create(element.Title, TweenInfo.new(0.25, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()
                            end
                        end
                    end
                end
        ...
        task.delay(0.45, function() Debounce = false Minimised = false end)
    end
    ...
    function RayfieldLibrary:Unhide()
        ...
        if Topbar then
            ...
            if Topbar:FindFirstChild("Title") then tween(Topbar.Title, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {TextTransparency = 0}) end
            if Topbar:FindFirstChild("ImageLabel") then tween(Topbar.ImageLabel, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {ImageTransparency = 0}) end
            if Topbar:FindFirstChild("UIStroke") then tween(Topbar.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Transparency = 0}):Play() end
        end
        task.delay(0.45, function() Debounce = false end)
    end

    -- Hook optional topbar buttons (if present), without altering existing connections
    pcall(function()
        if Topbar and Topbar:FindFirstChild("ChangeSize") and Topbar.ChangeSize:IsA("ImageButton") then
            Topbar.ChangeSize.MouseButton1Click:Connect(function()
                if Minimised then RayfieldLibrary:Maximise() else RayfieldLibrary:Hide(false) Minimised = true end
            end)
        end
        if Topbar and Topbar:FindFirstChild("Hide") and Topbar.Hide:IsA("ImageButton") then
            Topbar.Hide.MouseButton1Click:Connect(function()
                RayfieldLibrary:Hide(true)
            end)
        end
    end)
end)
---------------------------------------------------------------------
-- End of Beautification Patch
---------------------------------------------------------------------
