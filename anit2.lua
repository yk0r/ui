...
local PageLayout = Elements:FindFirstChildOfClass("UIPageLayout")

local function ensureUIScale(page)
    if not page:FindFirstChild("UIScale") then
...
local function switchTab(newPage)
    local oldPage = Elements.UIPageLayout.CurrentPage
    if oldPage == newPage then return end

    ensureUIScale(newPage)
    ensureUIScale(oldPage)

    local oldScale = oldPage.UIScale
    local newScale = newPage.UIScale
    ...
    Elements.UIPageLayout:JumpTo(newPage)
end
