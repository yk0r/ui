...
function RayfieldLibrary:CreateWindow(Settings)
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		TabButton.Interact.MouseButton1Click:Connect(function()
			...
			if currentPage and currentPage ~= TabPage then
				-- 保存原始大小和位置
				local originalSize = currentPage.Size
				local originalPosition = currentPage.Position
				
				-- 创建灵动岛风格的缩小淡出动画序列
				-- 第一阶段：缩小并淡出
				local shrinkTween = TweenService:Create(currentPage, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
				...
				-- 第二阶段：形变效果
				local morphTween = TweenService:Create(currentPage, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
				...
				shrinkTween.Completed:Connect(function()
					morphTween:Play()
					morphTween.Completed:Connect(function()
						-- 最终完全淡出
						TweenService:Create(currentPage, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			...
			if Elements.UIPageLayout.CurrentPage ~= TabPage then
				-- 先设置页面为初始状态（缩小且透明）
				TabPage.Size = UDim2.new(0.5, 0, 0.5, 0)
				TabPage.Position = UDim2.new(0.5, 0, 0.5, 0)
				TabPage.BackgroundTransparency = 1
				TabPage.Transparency = 1
				...
				-- 创建灵动岛风格的放大淡入动画序列
				-- 第一阶段：快速放大到中等大小
				local firstTween = TweenService:Create(TabPage, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
				...
				-- 第二阶段：形变效果
				local morphTween = TweenService:Create(TabPage, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
					Size = UDim2.new(0.95, 0, 1.05, 0)
				})
				
				-- 第三阶段：最终调整到正常大小
				local finalTween = TweenService:Create(TabPage, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
